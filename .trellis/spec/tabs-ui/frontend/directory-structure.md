# tabs-ui — PLACEHOLDER SPEC

**Expected package:** `@vben-core/tabs-ui` — Draggable tab chrome — `TabsView`, `TabButton`, context menu (close left/right/others), tab persistence to preferences. Side-effect when reordering routes.
...
### Typical src/ layout (varies by role — `tabs`)
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben-core/tabs-ui/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben-core/tabs-ui/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/tabs-ui/`
