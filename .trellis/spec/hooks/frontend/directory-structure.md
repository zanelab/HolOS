...
```
@vben/hooks/
├── package.json                        # name "@vben/hooks" v5.7.0
...
### Typical src/ layout (varies by role — `app-hooks`)
...
| Role (app-hooks) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/hooks/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/hooks/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/hooks/`
