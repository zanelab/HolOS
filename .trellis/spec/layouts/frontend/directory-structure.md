...
```
@vben/layouts/
├── package.json                        # name "@vben/layouts" v5.7.0
...
### Typical src/ layout (varies by role — `layouts`)
...
| Role (layouts) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/layouts/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/layouts/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/layouts/`
