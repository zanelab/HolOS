# base — PLACEHOLDER SPEC

**Expected package:** `@vben-core/base` — Lowest-level shared Vue 3 SFCs and TS types reused by every other `@vben-core` package — base buttons, transitions, app-loading wrapper, simple icons.
...
### Typical src/ layout (varies by role — `ui-primitives`)
...
| Role (ui-primitives) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben-core/base/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben-core/base/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/base/`
