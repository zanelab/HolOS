# plugins — PLACEHOLDER SPEC

**Expected package:** `@vben/plugins` — Vite plugin adapters — `@vben/plugins/vite`, `imports`, `auto-import`, `components`, `html`, `svg-icons-loader`, `router-layouts`. Dev-time only.
...
### Typical src/ layout (varies by role — `vite-plugins`)
...
| Role (vite-plugins) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/plugins/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/plugins/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/plugins/`
