...
```
@vben-core/form-ui/
├── package.json                        # name "@vben-core/form-ui" v5.7.0
...
### Typical src/ layout (varies by role — `form`)
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben-core/form-ui/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben-core/form-ui/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/form-ui/`
