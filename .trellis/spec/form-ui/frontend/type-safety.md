...
### 1. Generic utility types (core of `@vben-core/form-ui` for typings role)
...
pnpm typecheck --filter @vben-core/form-ui       # this package only
```
...
- ❌ Don't re-export types from `node_modules` directly — wrap with a
  `@vben-core/form-ui`-scoped alias.
