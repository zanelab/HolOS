...
### 1. Generic utility types (core of `@vben-core/layout-ui` for typings role)
...
pnpm typecheck --filter @vben-core/layout-ui       # this package only
```
...
- ❌ Don't re-export types from `node_modules` directly — wrap with a
  `@vben-core/layout-ui`-scoped alias.
