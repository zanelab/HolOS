# layout-ui — PLACEHOLDER SPEC

**Expected package:** `@vben-core/layout-ui` — VbenLayout primitives — `LayoutSidebar`, `LayoutHeader`, `LayoutTabbar`, `LayoutContent`, `LayoutFooter`, `LayoutBreadcrumb`, `LayoutMenu`. Styleless structure.
...
### Typical src/ layout (varies by role — `layout-primitives`)
...
| Role (layout-primitives) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben-core/layout-ui/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben-core/layout-ui/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/layout-ui/`
