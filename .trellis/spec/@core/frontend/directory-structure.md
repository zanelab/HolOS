# @core — PLACEHOLDER SPEC

**Expected package:** `@vben-core` — Composite barrel — re-exports all internal @vben-core/* primitives (base, typings, composables, design, form/layout/menu/popup/shadcn/tabs UI).
...
### Typical src/ layout (varies by role — `composite`)
...
| Role (composite) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben-core/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben-core/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/@vben-core/`
