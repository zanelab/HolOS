# web-naive Component Guidelines

> Vue 3 + vue + naive-ui conventions.

## Conventions

- **`<script setup lang="ts">`** only
- **`naive-ui`** components are imported **only** through `src/adapter/`
...
## `naive-ui` Adapter Style

```ts
// src/adapter/component/index.ts (sample)
import { message, notification } from 'naive-ui';
...
- ❌ Don't import `naive-ui` directly from views
