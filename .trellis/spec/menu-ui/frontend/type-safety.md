...
### 1. Generic utility types (core of `@vben-core/menu-ui` for typings role)
...
pnpm typecheck --filter @vben-core/menu-ui       # this package only
```
...
- ❌ Don't re-export types from `node_modules` directly — wrap with a
  `@vben-core/menu-ui`-scoped alias.
