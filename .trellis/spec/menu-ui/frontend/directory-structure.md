...
```
@vben-core/menu-ui/
├── package.json                        # name "@vben-core/menu-ui" v5.7.0
...
### Typical src/ layout (varies by role — `menu`)
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben-core/menu-ui/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben-core/menu-ui/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/menu-ui/`
