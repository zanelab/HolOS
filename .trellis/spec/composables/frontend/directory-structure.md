...
```
@vben/composables/
├── package.json                        # name "@vben/composables" v5.7.0
...
### Typical src/ layout (varies by role — `composables`)
...
| Role (composables) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/composables/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/composables/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/composables/`
