# design — PLACEHOLDER SPEC

**Expected package:** `@vben/design` — Design tokens — Tailwind preset, CSS variables, theme primitives (colors, spacing, radii, shadows), shadcn theme integration, dark/light themes.
...
### Typical src/ layout (varies by role — `tokens`)
...
| Role (tokens) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/design/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/design/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/design/`
