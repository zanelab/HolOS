# typings — PLACEHOLDER SPEC

**Expected package:** `@vben/typings` — Generic TS types — `ComponentRecord`, `Recordable<T>`, `Nullable<T>`, `ElementOf<T>`, `DeepPartial<T>`, `AnyFunction`, `AnyObject`, `PromiseFn`. Zero runtime.
...
### Typical src/ layout (varies by role — `typings`)
...
| Role (typings) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/typings/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/typings/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/typings/`
