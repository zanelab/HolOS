...
### 1. Generic utility types (core of `@vben-core/shadcn-ui` for typings role)
...
pnpm typecheck --filter @vben-core/shadcn-ui       # this package only
```
...
- ❌ Don't re-export types from `node_modules` directly — wrap with a
  `@vben-core/shadcn-ui`-scoped alias.
