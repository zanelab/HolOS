# common-ui — PLACEHOLDER SPEC

**Expected package:** `@vben/common-ui` — Fallback pages & auth chrome — about, profile, fallback-page, AuthenticationLoginExpiredModal, AuthenticationExpiredModal helpers. Reusable across all apps.
...
### Typical src/ layout (varies by role — `fallback-pages`)
...
| Role (fallback-pages) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/common-ui/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/common-ui/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/common-ui/`
