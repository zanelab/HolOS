...
```
@vben-core/popup-ui/
├── package.json                        # name "@vben-core/popup-ui" v5.7.0
...
### Typical src/ layout (varies by role — `popups`)
...
| Role (popups) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben-core/popup-ui/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben-core/popup-ui/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/popup-ui/`
