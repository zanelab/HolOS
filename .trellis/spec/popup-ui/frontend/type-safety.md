...
### 1. Generic utility types (core of `@vben-core/popup-ui` for typings role)
...
pnpm typecheck --filter @vben-core/popup-ui       # this package only
```
...
- ❌ Don't re-export types from `node_modules` directly — wrap with a
  `@vben-core/popup-ui`-scoped alias.
