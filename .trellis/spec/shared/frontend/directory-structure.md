# shared — PLACEHOLDER SPEC

**Expected package:** `@vben/shared` — Runtime constants shared between packages — error codes, polling intervals, default timeouts, locale codes. Different from `@vben/constants` (compile-time values).
...
### Typical src/ layout (varies by role — `runtime-constants`)
...
| Role (runtime-constants) | Extra dirs expected |
|---|---|
...
- **Public barrel** — `src/index.ts` is the **only** import entry point.
  Deep imports like `@vben/shared/internal/x` are forbidden.
...
- ❌ Don't `import` from `@vben/shared/internal/*` — only the public barrel.
- ❌ Don't create the actual `packages/shared/`
